...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/wydy/common build/common
...
git clone -b main --depth 1 https://github.com/wydy/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (wydy/build-actions|wydy/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/wydy/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/wydy/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="wydy/build-actions"
  sudo rm -rf build common.sh
