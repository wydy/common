...
cd ${GITHUB_WORKSPACE}
bash <(curl -fsSL https://raw.githubusercontent.com/wydy/common/main/custom/first.sh)
}
...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/wydy/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
adguardhome,luci-app-adguardhome,mosdns,luci-app-mosdns,luci-app-openclash, \
...
if [[ "${REPO_BRANCH}" =~ (19.07|21.02|22.03) ]]; then
  if [[ -d "${HOME_PATH}/feeds/passwall3/shadowsocksr-libev" ]]; then
    curl -o ${HOME_PATH}/feeds/passwall3/shadowsocksr-libev/Makefile https://raw.githubusercontent.com/wydy/common/main/Share/shadowsocksr-libev/Makefile
  fi
  # 降低shadowsocks-rust版本,最新版本编译不成功
  if [[ -d "${HOME_PATH}/feeds/passwall3/shadowsocks-rust" ]]; then
    curl -o ${HOME_PATH}/feeds/passwall3/shadowsocks-rust/Makefile https://raw.githubusercontent.com/wydy/common/main/Share/shadowsocks-rust/Makefile
  fi
fi
}
...
if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
  ...
  wget -q https://github.com/wydy/common/releases/download/API/AdGuardHome.api -O AdGuardHome.api
  if [[ $? -ne 0 ]];then
    curl -fsSL https://github.com/wydy/common/releases/download/API/AdGuardHome.api -o AdGuardHome.api
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/wydy/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
